...
async def very_compute_intensive():
    a = []
    a.append(1)
    a.append(1)
    for i in range(2,10):
...
async def start(loop, thread_parameters):
    tasks = []
    for i in range(3):
        tasks.append(loop.create_task(call_script(thread_parameters[i]['id'],thread_parameters[i]['arg'])))
    tasks.append(loop.create_task(very_compute_intensive()))
    for task in tasks:
        result = await task
        print(result)
